...
;
        ...
        if (page instanceof DownloadablePage) {
            ...
            if (null != downloads) {
                downloads.stream()//Possible to use parallelStreams but there is limited capacity on the host
                         .filter(d -> null != d)
                         .filter(d -> isNotCompressed(d.getFile()))
                         .forEach(dwnlds -> {
    ...
    private boolean isNotCompressed(final String file) {
        boolean compressed = FileContentExtractUtil.isCompressed(file);
        if (compressed) {
            LOGGER.info("isNotCompressed([file]) : file {} is compressed", file);
        }
        return !compressed;
